...
//                Event fire for new ticket['']
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status)
    {
        ...
        $tomorrow = \Carbon\Carbon::tomorrow(new DateTimeZone('Asia/Kolkata'));
        
        $current_day = $now->format('D');
        $currenTime = $now->format('H:m:s');

        $validTime = $now->format('19:00:00');

        $monDate = new DateTime();
        $monDate->modify('next monday');
        $mondayDate = $monDate->format('Y-m-d 10:00:00');
        
        $nextDay = $tomorrow->format('Y-m-d 10:00:00');
       
        $ovdate_n = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ovdate, 'Asia/Kolkata');
        // dd($nextDay);
        
        if ($current_day == 'Sat' || $current_day == 'Sun' || ($current_day == 'Fri' && $currenTime > $validTime)) { 
            $new_date = date_add(\Carbon\Carbon::parse($mondayDate), date_interval_create_from_date_string($sla_plan->grace_period));
        }

        else if ($currenTime > $validTime) {  
            $new_date = date_add(\Carbon\Carbon::parse($nextDay), date_interval_create_from_date_string($sla_plan->grace_period));
        }        
        else {
            $new_date = date_add(\Carbon\Carbon::parse($ovdate)->addHours(5)->addMinutes(30), date_interval_create_from_date_string($sla_plan->grace_period));
        }
