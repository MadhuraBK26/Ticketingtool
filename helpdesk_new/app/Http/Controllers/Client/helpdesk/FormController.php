	...
	/*	
        ...
        if (System::first()->status == 1) {
           $topics = $topic->get();
             ...
             $parentTopics = \DB::table('help_topic')
                                  ->where('parent_topic', '=', '')
                                  ->get();
           // dd($request->all());
           // $name = Request::input('name');
            // dd(Input::get('help_topic'));

            // // dd($parentTopics);
              // $topicsArray = array(); $i=0;
               $topicsArray[] = 'Select Query Topic';                   
               $subTopicsArray[] = 'Select Sub topic';
               $childTopicArray[] = 'Select sub topic';
/*                $childTopics = \DB::table('help_topic')->select('id','topic')->where('parent_topic', '=', 1)->get();
                foreach($childTopics as $child) {
                     $subTopicsArray[$child->id]= $child->topic;
                 }*/
            ...
            // //     $subchildTopics = \DB::
            // //      dd($childTopics);
                 $topicsArray[$parent->id] = $parent->topic;
            ...
            // }
                                 ...
                                 ///////////////////////////////////////
            // dd($topicsArray);
            // $parentTopics = \DB::table('help_topic')->where('parent_topic', '=', '')->get();
            //////////////////////////////////////////////////
            // $topicsArray = array(); $i=0;
            // foreach($parentTopics as $parent) {

            //     $childTopics = \DB::table('help_topic')->select('topic')->where('parent_topic', '=', $parent->topic)->get();
                
            //     foreach($childTopics as $child) {
            //         $topicsArray[$parent->topic][] = $child->topic;
            //         //$i++;
            //     }
            // //     dd($childTopics, $parent->parent_topic,$parent);
            // }
           ...
           // dd($topicsArray1);
            ...
            return view('themes.default1.client.helpdesk.form', compact('topicsArray', 'topics', 'codes', 'email_mandatory','subTopicsArray'))->with('phonecode', $phonecode);
        } else {
            return \Redirect::route('home');
        }
    }

    public function getSubHelptopic($parentId)  
    {
        $childTopics = \DB::table('help_topic')
                                 ->where('parent_topic', '=', $parentId)
                                 ->get();
    ...
    public function postedForm(User $user, ClientRequest $request, Ticket $ticket_settings, Ticket_source $ticket_source, Ticket_attachments $ta, CountryCode $code)
    {
        ...
        try {
            $form_extras = $request->except('Name', 'Phone', 'Email', 'Subject', 'Details', 'help_topic', '_wysihtml5_mode', '_token', 'mobile', 'Code', 'priority', 'attachment');
            $name = $request->input('Name');
...
//                    }
		...
		Session::flash('success', Lang::get('lang.Ticket-has-been-created-successfully-your-ticket-number-is').' '.$result[0].'. '.Lang::get('lang.Please-save-this-for-future-reference'));
		return Redirect::back();
