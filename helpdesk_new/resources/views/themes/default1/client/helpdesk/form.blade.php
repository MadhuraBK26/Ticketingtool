 ...
 <div class="col-md-12 form-group">
             ...
             <div class="form-inline">
               <div class="form-group">
                           ...
                           {!! Form::label('help_topic',Lang::get('lang.choose_a_help_topic')) !!}  
                            {!!Form::select('parent_topic', $topicsArray) !!}
          ...
          <div class="form-group">
                         ...
                         {!! Form::label('help_topic',Lang::get('lang.sub_topic')) !!}  
                            {!!Form::select('sub_parent_topic', $subTopicsArray) !!}
           ...
           <div class="form-group">
                            ...
                            {!!Form::select('help_topic', $subTopicsArray) !!}
...
<script type="text/javascript">
    $(document).ready(function() {
        $('select[name="parent_topic"]').on('change', function() {            
            var parentId = $(this).val();
            if(parentId) {
                $.ajax({
                    url: '/getSubHelptopic/'+parentId,
                    type: "GET",
                    dataType: "json",
                    success:function(data) {
                        $('select[name="sub_parent_topic"] option').remove();
                        console.log(data.length);
                        if (data.length > 0) {
                        $.each(data, function(key, value) {
                            $('select[name="sub_parent_topic"]').append('<option value="'+ value.id +'">'+ value.topic +'</option>');
                        });
                        } else {                           
                            $('select[name="sub_parent_topic"]').append('<option value="">Select Sub topic</option>');
                            $('select[name="help_topic"] option').remove();
                            $('select[name="help_topic"]').append('<option value="">Select Sub topic</option>');
        ...
        $('select[name="sub_parent_topic"]').on('change', function() {
            var parentId = $(this).val();
            if(parentId) {
                $.ajax({
                    url: '/getSubHelptopic/'+parentId,
                    ...
                    success:function(data) {
                        ...
                        if (data.length > 0) {
                            // alert("hhhh");
                        $.each(data, function(key, value) {
                            $('select[name="help_topic"]').append('<option value="'+ value.id +'">'+ value.topic +'</option>');
                        });
                        } else {
                            $('select[name="help_topic"] option').remove();
                            $('select[name="help_topic"]').append('<option value="">Select Sub topic</option>');
